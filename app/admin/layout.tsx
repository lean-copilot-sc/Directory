...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    // Safe-guard and check authorization
    if (!user || user.role === 'User') {
        const title = !user ? "Authentication Required" : "Access Denied";
        const message = !user
        ...
        return (
            <div className="min-h-screen bg-background flex flex-col items-center justify-center text-center p-4">
                ...
                <h1 className="text-3xl font-serif text-foreground mb-2 z-10">{title}</h1>
                <p className="text-muted max-w-md mb-8 z-10">{message}</p>
                <Link href={!user ? "/login" : "/"} className="z-10">
                    <Button variant="outline">{!user ? "Go to Login" : "Return to Directory"}</Button>
                </Link>
            </div>
        );
    }
